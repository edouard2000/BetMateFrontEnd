...
const GeneralChatScreen = ({ navigation }) => {
    ...
    useEffect(() => {
        ...
        socket.on('messages', (data) => {
            setMessages(data.data);
            // 667686b8cafc558ed66a811a is to be replaced by actual userId
            const unreadMessages = data.data.filter(msg => !msg.readBy.includes('66768003bc6550fbf2a04d06'));
    ...
    const sendMessage = async () => {
        if (inputText.trim()) {
            socket.emit('sendMessage', { userId: '66768003bc6550fbf2a04d06', message: inputText });
            setInputText('');
        }
    };
  }, []);
  ...
  const sendMessage = async () => {
    ...
    const markMessagesAsRead = () => {
        ...
        if (unreadMessageIds.length > 0) {
            socket.emit('markAsRead', { userId: '66768003bc6550fbf2a04d06', messageIds: unreadMessageIds });
            setUnreadCount(0);
        }
    };
